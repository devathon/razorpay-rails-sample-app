== Razorpay Rails Sample App

Rails sample app for integrating Razorpay API. Here is the demo[http://razorpay-sample.herokuapp.com/]

=== Installation
Put this line in your Gemfile:
  gem 'razorpay'
And then execute:
  $ bundle install
Or install it yourself as:
  $ gem install razorpay


=== Setup
You need Razorpay Merchant key and secret to use Razorpay API, to get sign up here[https://dashboard.razorpay.com/#/access/signup]

You can find your API keys at here[https://dashboard.razorpay.com/#/app/keys].

Create a file at +config/initializers/razorpay.rb+ and insert
  Razorpay.setup("merchant_key_id", "merchant_key_secret")
  
=== Usage

==== Checkout form
We can make the payment by two ways, either using simple form or javascript. see docs[https://docs.razorpay.com/docs/checkout-form]

or see here +/app/views/products/index.html.erb+

==== Payment Handling at backend

* ====  Fetch
    Razorpay::Payment.fetch(payment_id)
* ==== Capture
    Razorpay::Payment.fetch(payment_id).capture
    Razorpay::Payment.fetch(payment_id).capture({amount: product_price})
* ==== Refund
    Razorpay::Payment.fetch(payment_id).refund
    Razorpay::Payment.fetch("pay_29QQoUBi66xm2f").refund({amount: refund_amount})
* ==== Refunds
    Razorpay::Payment.fetch("pay_29QQoUBi66xm2f").refunds.all
    Razorpay::Payment.fetch(payment_id).refunds.all({from:Timestamp,to:Timestamp,count:2,skip:1})
  
You can checkout all entities returned by the Razorpay here[https://docs.razorpay.com/docs/return-objects]


=== Source
The reference used for this app https://docs.razorpay.com/v1/docs
